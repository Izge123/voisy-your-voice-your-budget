...
2. Для каждой транзакции определи: сумму, тип (income/expense/savings), категорию (матчи по названию из списка), описание.
...
4. ВАЖНО: Определи тип транзакции по смыслу:
   ...
   - СБЕРЕЖЕНИЯ (savings): отложил, положил в копилку, накопил, сберёг, отложенное, в резерв, на накопления
   - ВАЖНО: Если пользователь говорит об ОТКЛАДЫВАНИИ денег (отложил, положил в сбережения, в копилку, в резерв), это ВСЕГДА savings, даже если он упоминает источник (зарплату).
   - Если тип не ясен из контекста, по умолчанию это расход (expense).
5. Верни JSON в формате: { "transactions": [{ "amount": number, "category_id": string|null, "type": "income"|"expense"|"savings", "description": string }] }
...
Ввод: "Отложил 10000 рублей в сбережения"
Вывод: { "transactions": [{ "amount": 10000, "category_id": "<id категории Сбережения/Отложенное>", "type": "savings", "description": "Отложено в сбережения" }] }`;
